            ...
            if raw_result != "Error!" {
                ...
                match value.r#type {
                    ...
                    Type::RNAME => {
                        ...
                        self.send(Message::text(Command::new(value.r#type,Value::String(json!({"name": room_id, "size": room.get_size().await.to_string(), "members": room.list_of_member_names().await}).to_string()), user_id.clone()).to_string()));
                    }
                    ...
                    Type::JOIN => {
                        ...
                        if !helper::empty_data(&name){
                            ...
                            self.set_room_name(&name).await;
                            let users = new_room.member_iter().await.iter().cloned().collect::<Vec<(String,Arc<Client>)>>();
                            let message = Command::new(Type::JOIN,Value::String(format!("{} has joined the room {}", user_name,name).to_string()),user_id.clone()).to_string();
                            helper::broadcast_msg(Message::text(message), users)
                        }else {
                            // data is too small
                            self.send(Message::text(Command::error().to_string()))
                        }
